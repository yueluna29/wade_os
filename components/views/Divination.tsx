  ...
  return (
    <div className="h-full overflow-y-auto bg-wade-bg-app px-4 md:px-8 pt-6 pb-24 relative">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8">
        {[
          { title: 'Daily Pull',      desc: 'One card guidance',  iconUrl: driveUrlFromId('1sm0pmwshluhw6tcUt6AxnOyObuLkw-2O'), icon: 1 },
          { title: 'Spread Room',     desc: 'Explore spreads',    iconUrl: driveUrlFromId('1TgenbpIgZYl3wrVO_AcEsam4skFs-kPK'), icon: 2 },
          { title: 'Card Library',    desc: 'Browse all cards',   iconUrl: driveUrlFromId('1bpNyriOsg2sNM1xuGjvlYQxzOsfrmtun'), icon: 3 },
          { title: 'Reading Archive', desc: 'Your past readings', iconUrl: driveUrlFromId('1bWoSwVkg62PpPAYyDub3170ESQeiZdRx'), icon: 4 },
        ].map((item, idx) => (
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 items-start">
        ...
        <div className="lg:col-span-2 bg-wade-bg-card rounded-[2rem] p-5 md:p-6 shadow-sm border border-wade-border relative">
          ...
          <div className="mt-8 flex flex-row gap-4 md:gap-8 items-start">
            <div className="w-[40%] max-w-[160px] md:max-w-[240px] lg:max-w-[280px] shrink-0 relative">
              <div className="rounded-xl md:rounded-2xl overflow-hidden shadow-md ring-2 md:ring-4 ring-wade-bg-app relative z-10">
                <img src={todayCardImage} alt="The High Priestess" className="w-full h-auto block" />
              </div>
            </div>
