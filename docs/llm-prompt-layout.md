...
- **Gemini 路径**：如果有 `customPrompt`，会**预置**到 user message 前面，包 XML：
  ```
  <special_instructions priority="highest">
  {customPrompt}
  </special_instructions>

  {current message}
  ```
  （aiService.ts:288-295）
- **OpenAI 路径**：`customPrompt` 作为**独立的 system message**塞在 history 最后、current user message 前面，同样包 `<special_instructions>` XML（aiService.ts:319）
...
{{wadeosData}}:                                     ← Wade 的"手机数据"
  ...
  - Your memories about Luna                       ← 向量检索 top-7（anchor = summary + 最近 Luna 消息）；失败回退到最近 5 条
